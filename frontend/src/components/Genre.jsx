...
const Genre = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
        try {
            const response = await axios.get('http://localhost:8080/api/v1/genres/' + id, {
            headers: {
                Authorization: `Bearer ${localStorage.getItem('token')}`
            }
            });
    ...
    return ( 
        ...
        <h2 style={{color: '#2c3e50', paddingLeft: '100px'}}>Genre: {genre}</h2>
            <div className="genre-page" style={{paddingLeft: '10%', display: 'flex', flexWrap: 'wrap', width: '1400px'}}>
